...
class TestGetPages:
    ...
    def test_get_restore_password_page_by_restore_password_button(self, driver):
        ...
        WebDriverWait(driver, 10).until(
            expected_conditions.element_to_be_clickable(LocatorsLoginPage.button_restore_password)).click()
    ...
    def test_restore_password(self, driver, create_user):
        ...
        WebDriverWait(driver, 10).until(
            expected_conditions.element_to_be_clickable(LocatorsLoginPage.button_restore_password)).click()
        WebDriverWait(driver, 10).until(
            expected_conditions.element_to_be_clickable(LocatorsResetPasswordPage.input_email_field_restore_password_page)).send_keys(user_data['user']['email'])
        WebDriverWait(driver, 10).until(
            expected_conditions.element_to_be_clickable(LocatorsResetPasswordPage.button_restore_password_restore_page)).click()
        WebDriverWait(driver, 10).until(
    ...
    def test_show_hide_button_underline_password_field(self, driver, create_user):
        ...
        WebDriverWait(driver, 10).until(
            expected_conditions.element_to_be_clickable(LocatorsLoginPage.button_restore_password)).click()
        WebDriverWait(driver, 10).until(
            expected_conditions.element_to_be_clickable(LocatorsResetPasswordPage.input_email_field_restore_password_page)).send_keys(
            user_data['user']['email'])
        WebDriverWait(driver, 10).until(
            expected_conditions.element_to_be_clickable(
                LocatorsResetPasswordPage.button_restore_password_restore_page)).click()
        WebDriverWait(driver, 10).until(
